<template>
  <div>
    <div class="a">
      你们一起打扫活动室卫生
      <span class="value"> {{ userData["cleannum"]}} </span>
      次
    </div>
    <div class="b">
      按时打扫
      <span class="value"> {{ userData["cleanintime"]}} </span>
      次
    </div>
...
export default {
  ...
  computed: {
    ...
    intimeRate(){
      return (this.$store.state.userData["cleanintime"]/this.$store.state.userData["cleannum"]) * 100
    }
  },
  methods:{
    numFilter (value) {
  ...
  data(){
    return {
      imgUrl:this.$store.state.userData["tup3"]
    }
  }
