...
<!--                    <p class="form-title&#45;&#45;heading"><span>Login to</span><span>Assignment Portal</span></p>-->
                ...
                <section class="form-login mt-6c">
                    <form v-on:submit.prevent="getdata" class="login-form">
                        <p v-if="incorrectAuth" class="bg-red-500 text-red-900 rounded-xl py-2 px-4 mb-6 alert-danger" style="width: 400px"><i class="fas fa-exclamation-triangle mr-2"></i>Not Find name or stuid</p>
                        <div class="form-component">
                            <input type="text" name="name" v-model="name" class="input-field" autocomplete="off" required>
                            <span class="label-placeholder">姓名</span>
                        </div>

                        <div class="form-component">
                            <input type="text" name="password" v-model="password" class="input-field" autocomplete="off" required>
                            <!-- <input type="password" name="password" v-model="password" class="input-field" autocomplete="off" required> -->
                            <span class="label-placeholder">学号</span>
                        </div>
    ...
    export default {
        ...
        methods: {
            ...
            getdata(){
                ...
                this.$store.dispatch('usergetData', {
                    name: this.name,
                    studyid: this.password
                })
                .then(() => {
                    console.log("then函数")
                    this.$router.push({ name: 'Home' });
